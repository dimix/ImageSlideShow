...
open class ImageSlideShowViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
	...
	private var originPanViewCenter			: CGPoint = .zero
	private var panViewCenter				: CGPoint = .zero
	private var navigationBarHidden			: Bool = false
	private var toggleBarButtonItem			: UIBarButtonItem?
	private var _currentIndex				: Int = 0
	private let slidesViewControllerCache	: ImageSlideShowCache = ImageSlideShowCache()
	...
	func setNavigationBar(visible:Bool) {
		...
		UIView.animate(withDuration: 0.23, delay: 0.0, options: .beginFromCurrentState,
			animations: {
	...
	private func indexOfSlideForViewController(viewController: UIViewController) -> Int {
		guard let viewController = viewController as? ImageSlideViewController
			else { fatalError("Unexpected view controller type in page view controller.") }
		guard let viewControllerIndex = indexOfProtocolObject(inSlideViewController: viewController)
			else { fatalError("View controller's data item not found.") }
	...
	private func slideViewController(forPageIndex pageIndex: Int) -> ImageSlideViewController? {
		if let slides = slides, slides.count > 0 {
			...
			if let cachedController = slidesViewControllerCache.object(forKey: slide.slideIdentifier() as AnyObject) as? ImageSlideViewController {
				...
				guard
					let controller = self.storyboard?.instantiateViewController(withIdentifier: "ImageSlideViewController") as? ImageSlideViewController
					else { fatalError("Unable to instantiate a ImageSlideViewController.") }
	...
	@objc
	private func tapGesture(gesture: UITapGestureRecognizer) {
